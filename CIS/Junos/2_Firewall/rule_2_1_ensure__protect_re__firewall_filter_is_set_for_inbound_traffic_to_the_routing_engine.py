...
def rule_2_1_ensure__protect_re__firewall_filter_is_set_for_inbound_traffic_to_the_routing_engine(configuration, commands, device, devices):
    ...
    if ipv4_configured:
        assert ipv4_filter_present, "IPv4 firewall filter missing on lo0 interface"
    ...
    if ipv6_configured:
        assert ipv6_filter_present, "IPv6 firewall filter missing on lo0 interface"
