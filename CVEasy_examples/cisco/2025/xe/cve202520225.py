...
def rule_cve202520225(configuration, commands, device, devices):
    """
    This rule checks for CVE-2025-20225 vulnerability in Cisco IOS XE Software.
    
    A vulnerability in the IKEv2 feature could allow an unauthenticated, remote 
    attacker to trigger a memory leak, resulting in a denial of service (DoS) 
    condition. The vulnerability is due to improper processing of IKEv2 packets.
    ...
    the device to reload unexpectedly.
    """
    # Extract the version information from the command output
    version_output = commands.show_version

    # List of vulnerable software versions for IOS XE
    # Based on the advisory, all versions with IKEv2 enabled are vulnerable until fixed
    vulnerable_versions = [
        # 16.x versions
    ...
    # Check if IKE is enabled by checking if UDP ports 500 or 4500 are open
    udp_output = commands.show_udp
    ike_enabled = ('500' in udp_output or '4500' in udp_output) and udp_output.strip() != ''

    # If IKE is not enabled, device is not vulnerable
    if not ike_enabled:
        return

    # Check if IKEv2 is being used by examining crypto map configuration
    ...
    assert not ikev2_enabled, (
        ...
        "The device is running a vulnerable version of IOS XE Software AND has IKEv2 enabled. "
