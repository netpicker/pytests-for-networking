...
def rule_cve202520239(configuration, commands, device, devices):
    """
    This rule checks for CVE-2025-20239 vulnerability in Cisco IOS XE Software.
    
    A vulnerability in the IKEv2 feature could allow an unauthenticated, remote 
    attacker to trigger a memory leak, resulting in a denial of service (DoS) condition.
    
    The vulnerability is due to improper processing of IKEv2 packets. An attacker 
    could exploit this by sending crafted IKEv2 packets to an affected device.
    ...
    # These are versions that need to be patched
    vulnerable_versions = [
        # 16.x versions
    ...
    # Check if IKE (v1 or v2) is enabled by checking if UDP port 500 or 4500 is open
    udp_output = commands.show_udp
    ike_enabled = ('500' in udp_output or '4500' in udp_output) and len(udp_output.strip()) > 0

    # If IKE is not enabled, the device is not vulnerable
    if not ike_enabled:
        return

    # Check if IKEv2 is being actively used by checking crypto map configuration
    crypto_map_output = commands.show_crypto_map
    ikev2_active = 'IKEv2 Profile' in crypto_map_output

    # If IKEv2 is active, the device is vulnerable
    assert not ikev2_active, (
        f"Device {device.name} is vulnerable to CVE-2025-20239. "
        "The device is running a vulnerable version of Cisco IOS XE Software AND has IKEv2 enabled. "
        "An unauthenticated, remote attacker could trigger a memory leak by sending crafted IKEv2 packets, "
