...
@medium(
    ...
    commands=dict(
        ...
        show_shell='show running-config | include shell'
    ),
)
def rule_cve202520149(configuration, commands, device, devices):
    """
    This rule checks for the presence of CVE-2025-20149 vulnerability in Cisco IOS XE Software.
    
    A vulnerability in the CLI of Cisco IOS Software and Cisco IOS XE Software could allow an 
    authenticated, local attacker to cause an affected device to reload unexpectedly, resulting 
    in a denial of service (DoS) condition.
    
    This vulnerability is due to a buffer overflow. An attacker with a low-privileged account 
    could exploit this vulnerability by using crafted commands at the CLI prompt.
    
    The vulnerability affects devices with 'shell processing full' command configured.
    """
    # Extract the version information from the command output
    version_output = commands.show_version

    # List of vulnerable software versions
    # Based on the advisory, this affects Cisco IOS and IOS XE Software
    # The advisory doesn't specify exact versions, so we check for common vulnerable versions
    vulnerable_versions = [
        # 16.x versions
    ...
    shell_output = commands.show_shell
    shell_processing_full_enabled = 'shell processing full' in shell_output

    # If shell processing full is enabled, the device is vulnerable
    assert not shell_processing_full_enabled, (
        f"Device {device.name} is vulnerable to CVE-2025-20149. "
        "The device is running a vulnerable version AND has 'shell processing full' configured. "
        "This vulnerability allows an authenticated, local attacker to cause a DoS condition via buffer overflow. "
        "Mitigation: Remove 'shell processing full' using 'no shell processing full' command. "
