...
def rule_cve202520173(configuration, commands, device, devices):
    """
    This rule checks for CVE-2025-20173 vulnerability in Cisco IOS XE Software.
    ...
    snmp_community_enabled = commands.show_snmp_community and 'snmp-server community' in commands.show_snmp_community
    snmp_group_enabled = commands.show_snmp_group and 'snmp-server group' in commands.show_snmp_group
    snmp_user_enabled = commands.show_snmp_user and 'User name:' in commands.show_snmp_user
    
    snmp_v3_enabled = snmp_group_enabled and snmp_user_enabled
    snmp_enabled = snmp_community_enabled or snmp_v3_enabled

    # If SNMP is enabled, the device is vulnerable
    assert not snmp_enabled, (
        ...
        "An authenticated, remote attacker could exploit this vulnerability by sending a crafted SNMP request "
        "to cause the device to reload unexpectedly, resulting in a DoS condition. "
        "Upgrade to a fixed release or disable SNMP if not required. "
