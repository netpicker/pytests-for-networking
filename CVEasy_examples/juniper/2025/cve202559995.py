...
def rule_cve202559995(configuration, commands, device, devices):
    """
    CVE-2025-59995: XSS in Junos Space Quick Template page allows script
    ...
    has_junos_space_config = (
        config_output.strip().startswith('set')
        and 'junos-space' in config_output
    )

    assert not has_junos_space_config, (
        f"Device {device.name} is vulnerable to CVE-2025-59995. "
        "Running a vulnerable Junos Space version (before 24.1R4) with "
        "Junos Space configured allows XSS in the Quick Template page. "
        "Upgrade to 24.1R4 or later. "
        "See https://supportportal.juniper.net/CVE-2025-59995"
    )
